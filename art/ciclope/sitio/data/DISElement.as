package art.ciclope.sitio.data {
	...
	public class DISElement {
		...
		public function DISElement(order:uint, id:String, type:String, time:uint, path:String) {
			// get data
			this._order = order;
			this._time = time;
			this._id = id;
		...
		/**
		 * Element position on playlist sequence
		 */
			...
			for (index = 0; index < to.child("file").length(); index++) {
				if (to.file[index].hasOwnProperty('@format') && to.file[index].hasOwnProperty('@lang') && to.file[index].hasOwnProperty('@absolute')) {
					...
					if ((this._type == DISFileFormat.MEDIA_TEXT) || (this._type == DISFileFormat.MEDIA_PARAGRAPH)) {
						this._file[String(to.file[index].@format)][String(to.file[index].@lang)] = new DISElementFile(String(to.file[index].@format), String(to.file[index].@lang), Boolean(uint(to.file[index].@absolute)), String(to.file[index]), this._path, this._type, String(to.file[index].@font), String(to.file[index].@fontStyle));
					} else {
						this._file[String(to.file[index].@format)][String(to.file[index].@lang)] = new DISElementFile(String(to.file[index].@format), String(to.file[index].@lang), Boolean(uint(to.file[index].@absolute)), String(to.file[index]), this._path, this._type);
					}
